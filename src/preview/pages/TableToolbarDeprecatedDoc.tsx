...
export function TableToolbarDeprecatedDoc() {
  ...
  return (
    <DocLayout toc={TOC}>
      <DocHeader
        category="Tables"
        title="Table Toolbar (Deprecated)"
        description="⚠️ **Deprecada** — layout legado (dumb, slots left/actions + chips). Mantida só pra não quebrar consumidores existentes (via `<DataTable deprecatedToolbar>`); será removida numa versão futura. Novos usos devem consumir a **Table Toolbar** canônica. Composição de partes (search, view tabs, segmented, tool buttons) + chips de filtros aplicados."
      />
      <DocSeparator />
...
/>`}
      ...
      <ExampleSection
        ...
        description="As partes podem ser usadas fora do TableToolbarDeprecated. Cada uma é dumb e cuida de uma responsabilidade só."
...
/>`}
      ...
      <p className="text-body-md text-fg-muted mb-gp-lg">
        <strong className="text-fg-default">TableToolbarDeprecated</strong> (root):
      </p>
      <PropsTable items={ROOT_PROPS} />

      <p className="text-body-md text-fg-muted mt-gp-2xl mb-gp-lg">
        Parts (exportadas de <code>ui/TableToolbarDeprecated</code>):
      </p>
