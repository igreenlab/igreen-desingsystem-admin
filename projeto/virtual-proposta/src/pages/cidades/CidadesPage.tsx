...
const sparkCfg = { v: { label: "Cadastros", color: "var(--color-chart-3)" } } satisfies ChartConfig;
...
const STATUS_KPI = [
  { key: "ativos", label: "Ativos", icon: UserCheck, iconClass: "bg-bg-success-muted text-fg-success" },
  { key: "licenc", label: "Licenc.", icon: BadgeCheck, iconClass: "bg-bg-info-muted text-fg-info" },
  { key: "aguard", label: "Aguard.", icon: Clock, iconClass: "bg-bg-warning-muted text-fg-warning" },
  { key: "devol", label: "Devol.", icon: RefreshCcw, iconClass: "bg-bg-warning-muted text-fg-warning" },
  { key: "cancel", label: "Cancel.", icon: XCircle, iconClass: "bg-bg-danger-muted text-fg-danger" },
  { key: "reprov", label: "Reprov.", icon: Ban, iconClass: "bg-bg-danger-muted text-fg-danger" },
] as const satisfies ReadonlyArray<{
  key: keyof Pick<CidadeRow, "ativos" | "licenc" | "aguard" | "devol" | "cancel" | "reprov">;
  label: string;
...
export function CidadesPage() {
  ...
  const columns = useMemo<DataTableColumnDef<CidadeRow>[]>(
    () => [
      {
        field: "rank",
        headerName: "Ranking",
      ...
      },
      {
        ...
        enableColumnFilter: true,
        filterType: "select",
        filterOptions: [...new Set(cidades.map((c) => c.uf))].map((u) => ({ value: u, label: u })),
      },
      {
        field: "total",
        headerName: "Total",
      ...
      { field: "ativos", headerName: "Ativos", width: 100, align: "right", sortable: true, type: "number", aggregate: "sum", render: ({ value }) => <span className="tabular-nums text-fg-success">{num(value as number)}</span> },
      { field: "licenc", headerName: "Licenc.", width: 100, align: "right", sortable: true, type: "number", aggregate: "sum", render: ({ value }) => <span className="tabular-nums text-fg-default">{num(value as number)}</span> },
      { field: "aguard", headerName: "Aguard.", width: 100, align: "right", sortable: true, type: "number", aggregate: "sum", render: ({ value }) => <span className="tabular-nums text-fg-warning">{num(value as number)}</span> },
      { field: "devol", headerName: "Devol.", width: 100, align: "right", sortable: true, type: "number", aggregate: "sum", render: ({ value }) => <span className="tabular-nums text-fg-warning">{num(value as number)}</span> },
      { field: "cancel", headerName: "Cancel.", width: 100, align: "right", sortable: true, type: "number", aggregate: "sum", render: ({ value }) => <span className="tabular-nums text-fg-danger">{num(value as number)}</span> },
      { field: "reprov", headerName: "Reprov.", width: 100, align: "right", sortable: true, type: "number", aggregate: "sum", render: ({ value }) => <span className="tabular-nums text-fg-danger">{num(value as number)}</span> },
    ],
    [],
  );

  const listConfig = useMemo<DataTableListConfig<CidadeRow>>(
    () => ({
      paginated: true,
  ...
  return (
    <div className="flex flex-col gap-gp-2xl">
      <PageHeader
        title="Cidades"
        description="Ranking de cidades por cadastros e status da carteira."
