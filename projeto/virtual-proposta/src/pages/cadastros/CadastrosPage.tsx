...
import { Calendar, ChevronDown, Check, MapPin, Hash, Zap, User, GraduationCap, Building2, CalendarDays, CircleDot } from "lucide-react";
...
const TABLE_VIEWS: DataTablePresetView[] = [
  presetView({
    id: "preset:pendentes",
    name: "Pendentes",
    filters: [{ field: "status", operator: "isAnyOf", value: ["analise", "aguardando", "devolucao"] }],
...
export function CadastrosPage() {
  ...
  const columns = useMemo<DataTableColumnDef<CadastroRow>[]>(
    () => [
      ...
      },
      {
        ...
        enableColumnFilter: true,
        filterType: "select",
        filterOptions: [...new Set(cadastros.map((c) => c.distribuidora))].map((d) => ({
          value: d,
          label: d,
        })),
      },
  ...
  return (
    <div className="flex flex-col gap-gp-2xl">
      <PageHeader
        title="Cadastros"
        description="Registros de clientes Green e seus estágios no fluxo de cadastro."
      ...
      <DataTable<CadastroRow>
        ...
        columns={columns}
        getRowId={(r) => r.id}
        persistId="cadastros-clientes-v2"
