...
import {
  ...
  PERIODOS,
  num,
  type TelecomCidadeRow,
  type KpiCard,
} from "./telecom-cidades-mock";

const sparkCfg = { v: { label: "Linhas", color: "var(--color-chart-3)" } } satisfies ChartConfig;
...
const STATUS_KPI = [
  { key: "ativas", label: "Ativas", icon: Signal, iconClass: "bg-bg-success-muted text-fg-success" },
  { key: "ativacao", label: "Ativação", icon: Smartphone, iconClass: "bg-bg-info-muted text-fg-info" },
  { key: "portabilidade", label: "Portab.", icon: ArrowLeftRight, iconClass: "bg-bg-warning-muted text-fg-warning" },
  ...
  { key: "prepago", label: "Pré-pago", icon: CreditCard, iconClass: "bg-bg-muted text-fg-muted" },
] as const satisfies ReadonlyArray<{
  key: keyof Pick<TelecomCidadeRow, "ativas" | "ativacao" | "portabilidade" | "suspensas" | "canceladas" | "prepago">;
  label: string;
  icon: ComponentType<{ className?: string }>;
  iconClass: string;
}>;

export function TelecomCidadesPage() {
  ...
  const columns = useMemo<DataTableColumnDef<TelecomCidadeRow>[]>(
    () => [
      {
        field: "rank",
        headerName: "Ranking",
      ...
      },
      {
        ...
        enableColumnFilter: true,
        filterType: "select",
        filterOptions: [...new Set(cidadesTelecom.map((c) => c.uf))].map((u) => ({ value: u, label: u })),
      },
      {
        field: "total",
        headerName: "Linhas",
      ...
      { field: "ativacao", headerName: "Ativação", width: 100, align: "right", sortable: true, type: "number", aggregate: "sum", render: ({ value }) => <span className="tabular-nums text-fg-info">{num(value as number)}</span> },
      { field: "portabilidade", headerName: "Portab.", width: 100, align: "right", sortable: true, type: "number", aggregate: "sum", render: ({ value }) => <span className="tabular-nums text-fg-warning">{num(value as number)}</span> },
      ...
      { field: "prepago", headerName: "Pré-pago", width: 100, align: "right", sortable: true, type: "number", aggregate: "sum", render: ({ value }) => <span className="tabular-nums text-fg-muted">{num(value as number)}</span> },
    ],
    [],
  );

  const listConfig = useMemo<DataTableListConfig<TelecomCidadeRow>>(
    () => ({
      paginated: true,
  ...
  return (
    <div className="flex flex-col gap-gp-2xl">
      <PageHeader
        title="Cidades"
        description="Ranking de cidades por linhas ativas e status da base."
      ...
      <DataTable<TelecomCidadeRow>
        rows={cidadesTelecom}
        columns={columns}
        getRowId={(r) => r.id}
        persistId="telecom-cidades"
