...
import {
  ...
  type PendenciaMotivo,
} from "./telecom-pendencias-mock";

const STATUS_CHIP: Record<PendenciaStatus, "warning" | "info" | "success" | "danger"> = {
  pendente: "warning",
  reenviado: "info",
  resolvido: "success",
  vencida: "danger",
...
/** Card único — número de destaque + barra segmentada por motivo + resolvidas na
 *  semana + faixa de alerta com ação. Título e subtítulo no topo do próprio card. */
    ...
    <section className="flex flex-col rounded-radius-lg border border-border-subtle bg-bg-surface p-pad-3xl shadow-sh-sm">
      <div className="flex items-start justify-between gap-gp-md">
        <div className="flex flex-col gap-gp-2xs">
          <p className="text-title-md font-semibold leading-tight text-fg-default">Pendências</p>
          <p className="text-body-sm text-fg-muted">
            Acompanhe motivos, prazos e a resolução das pendências.
          </p>
        </div>
        <MoreVertical className="size-icon-sm shrink-0 text-fg-muted" />
      </div>

      <p className="mt-pad-3xl text-caption-md text-fg-muted">Pendências em aberto</p>
      ...
      <p className="text-body-md font-semibold text-fg-default">
        {num(totalResolvidas)} pendências resolvidas esta semana
      </p>
      <div className="mt-pad-md flex items-center justify-between gap-gp-md rounded-radius-base bg-bg-muted px-pad-3xl py-pad-2xl">
        <span className="flex items-center gap-gp-sm text-body-sm text-fg-muted">
          ...
          {num(totalVencidas)} pendências vencidas — revisar antes de {fmtDayMonth(proximoPrazo)}.
        </span>
        <button
          type="button"
...
/** Ranking de licenciados com mais pendências — barras horizontais. */
    ...
    <section className="flex flex-col rounded-radius-lg border border-border-subtle bg-bg-surface p-pad-3xl shadow-sh-sm">
      <div className="flex items-start justify-between gap-gp-md">
        <div className="flex flex-col gap-gp-2xs">
          ...
          <p className="text-body-sm text-fg-muted">Pendências por licenciado no período.</p>
        </div>
        <MoreVertical className="size-icon-sm shrink-0 text-fg-muted" />
      </div>
...
export function TelecomPendenciasPage() {
  ...
  const columns = useMemo<DataTableColumnDef<PendenciaRow>[]>(
    () => [
      ...
      },
      {
        ...
        enableColumnFilter: true,
        filterType: "select",
        filterOptions: (Object.keys(MOTIVO_COLOR) as PendenciaMotivo[]).map((m) => ({
          value: m,
          label: m,
        })),
        render: ({ value }) => (
          <span className="flex items-center gap-gp-sm text-fg-default">
            <span
              ...
              style={{ background: MOTIVO_COLOR[value as PendenciaMotivo] }}
            />
            {value as string}
          </span>
        ),
      ...
      },
      {
        ...
        enableColumnFilter: true,
        filterType: "select",
        filterOptions: [...new Set(pendencias.map((d) => d.uf))].map((u) => ({ value: u, label: u })),
      },
      {
        field: "licenciado",
  ...
  return (
    <div className="flex flex-col gap-gp-2xl">
      <PageHeader
        title="Pendências"
        description="Cadastros Telecom com pendência — motivo, prazo e resolução."
        actions={periodSelector}
      />

      {/* KPI — card visão geral + ranking */}
      <div className="grid grid-cols-1 items-stretch gap-gp-2xl lg:grid-cols-2">
        <VisaoGeralCard />
        <RankingCard />
      </div>

      {/* Manejo — tabela das pendências */}
      <DataTable<PendenciaRow>
        rows={pendencias}
        columns={columns}
        getRowId={(r) => r.id}
        persistId="telecom-pendencias"
