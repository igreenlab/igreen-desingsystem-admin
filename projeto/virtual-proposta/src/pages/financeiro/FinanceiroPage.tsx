...
const TABLE_VIEWS: DataTablePresetView[] = [
  presetView({ id: "preset:vencidos", name: "Vencidos", filters: [{ field: "status", value: "vencido" }] }),
  presetView({ id: "preset:disponiveis", name: "Disponíveis", filters: [{ field: "status", value: "disponivel" }] }),
  presetView({ id: "preset:pagos", name: "Pagos", filters: [{ field: "status", value: "pago" }] }),
];
      ...
      },
      {
        ...
        render: ({ row }) => (
          <Button
            ...
            onClick={(e) => {
              e.stopPropagation();
              // TODO: redirecionar para a visualização do boleto (row.id)
            }}
          >
            Abrir
          </Button>
          ...
          <div className="flex w-full items-center gap-gp-lg">
            <div className="flex min-w-0 flex-1 flex-col gap-gp-2xs">
              ...
              <span className="truncate text-caption-md text-fg-muted">
                {row.cidade}/{row.uf} · {row.licenciado} · vence {dateBR(row.vencimento)}
              </span>
            </div>
    ...
    <div className="flex flex-col gap-gp-2xl">
      ...
      <section className="rounded-radius-xl border border-border-subtle bg-bg-surface shadow-sh-sm">
        <div className="grid grid-cols-1 divide-y divide-border-subtle sm:grid-cols-2 sm:divide-y-0 lg:grid-cols-4 [&>*]:border-border-subtle sm:[&>*]:border-l sm:[&>*:first-child]:border-l-0 lg:[&>*:nth-child(3)]:border-l">
          <KpiCell icon={Receipt} label="Com boleto gerado" value={num(emProducao)} hint="carteira faturada" />
          <KpiCell icon={CheckCircle2} label="Boletos pagos" value={num(carteira.pagos)} hint={`${dec1(pctAdimplencia)}% adimplência`} />
          <KpiCell icon={Clock} label="Disponível (a vencer)" value={num(carteira.disponivel)} hint="boleto gerado, em aberto" />
          <KpiCell icon={AlertTriangle} label="Vencidos" value={num(carteira.vencidos)} hint={`${dec1(pctInadimplencia)}% inadimplência`} highlight />
        </div>
      </section>
