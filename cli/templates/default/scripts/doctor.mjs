 ...
 * Resync em qualquer caso: `npm run igreen:update -- utils tv --force`
 *
...
for (const { local, item, endsWith } of CHECKS) {
  if (!existsSync(local)) {
    console.error(`✗ ${local} AUSENTE — npm run igreen:add -- ${item}`);
    ok = false;
    continue;
  }
  ...
  try {
    ...
    if (hash(remote) === localHash) {
      ...
      console.error(`  Resync: npm run igreen:update -- ${item} --force`);
      ok = false;
    }
  } catch (e) {
