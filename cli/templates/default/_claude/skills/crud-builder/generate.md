...
2. **Coluna `actions` por ÚLTIMO**, `type: "actions"` (via `actionColumn`/`getActions`). O DataTable **ancora à direita por default**, então `pinned: "right"` é redundante. ⚠️ `width` é o caso à parte: `actions` é tipo **estrutural** e não está no registry, logo não tem `defaultWidth` — com `autoFit` (default ON) o autoFit resolve, mas o exemplo canônico fixa `width: 64` de propósito, e espelhar isso é seguro. O que não precisa é o `pinned`.
