...
| **painel de DETALHE** (o que abre ao clicar na linha da tabela ou no card) | **cite um bloco da família `dsgreen-paneldetail-*`** — a estrutura já está resolvida em três arranjos: **`-1`** registro com muitos campos (seções colapsáveis por assunto, sem aba) · **`-2`** tarefa (título grande no corpo, propriedades em lista plana com ícone, abas pra anexo/comentário/log) · **`-3`** com tabela dentro (ficha em 2 colunas + faixa de métricas + tabela compacta, e aí o painel é `xl` 720px). Em todos: identidade ou contexto no header, ações de ícone soft, ação primária no footer, e nunca botão de ação solto no corpo. Precisa de avatar/chip/ação no header → é `floating-panel`, porque no `panel` o header é só string |
