...
- **L-057** Largura de página é **`max-w-page-*`** (768px = `max-w-page-md`); overlay é `max-w-modal-*`/`max-w-drawer-*`/`max-w-tooltip-*`. Até 2026-09-23 a escala de página ocupava os nomes NATIVOS (`max-w-md` valia 768, `max-w-lg` valia 1024): override silencioso que deu 70 overlays com o dobro da largura num consumidor. **`max-w-md` agora é o Tailwind (448px)** e `max-w-container-*` nunca existiu. Gate: `dead-theme-classes`.
