...
| **NavTabs** | `ui/NavTabs/nav-tabs.styles.ts` | `src/components/ui/NavTabs/` | ✅ **vNEXT** — abas de NAVEGADOR (sessão aberta: conversa, chamado, registro), compound: `NavTabs` + `.Tab` `.Title` `.Subtitle` `.Action` `.Actions` `.Panel`. Controlado (`value`/`onValueChange`) e **não hospeda o conteúdo** — painel pode morar fora (`panelId` → `aria-controls`). `density` (48/40px) · `fill` (altura total, une por cor em vez de régua) · `actionsMode` · overflow com setas + lista. Não confundir com o `tabs` shadcn, que troca miolo dentro da tela |
