...
| ScreenLoader | `ui/ScreenLoader/screen-loader.styles.ts` | `src/components/ui/ScreenLoader/` | ✅ implementado — loading de página/área que preenche o pai (irmão do EmptyState): `variant="spinner"` (Spinner centrado + título + descrição, default) ou `variant="skeleton"` (silhueta genérica de página, sem prever layout). sizes sm/md/lg. Registry: pendente (`/ds-release`). Doc `#/screen-loader` |
