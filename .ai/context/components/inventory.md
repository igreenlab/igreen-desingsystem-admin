...
| Scheduler | `ui/Scheduler/scheduler.styles.ts` | `src/components/ui/Scheduler/` | 🚧 **em construção** — calendário de EVENTOS (não confundir com `Calendar`, que é seletor de data). View `month` completa (grade 6×7, navegação, busca, filtros declarativos por categoria/tag/cor, `+N mais`, `onEventClick`, `+` de criar); views `week`/`day`/`list` renderizam placeholder; drag & drop e teclado ainda não implementados. Núcleo puro (`hooks/layout.ts`: lane-packing, spans multi-dia, snap, resize) pronto e coberto por 36 testes. Dumb sobre mutação: emite `onEventMove`/`onEventResize`, nunca muta `events`. Dep: `date-fns` — **ainda não registrado no registry.json** (fecha no `/ds-release`) |
