...
| **ColorPicker** | `ui/ColorPicker/color-picker.styles.ts` | `src/components/ui/ColorPicker/` | ✅ **v0.10.x** — seletor de cor hex (#RRGGBB) controlado. Trigger = swatch (bg dinâmico via inline style — exceção L-027) + Input hex; abre Popover com grid de presets (paleta iGreen curada em `color-picker.presets.ts`) + hex livre. Composto de `Popover` + `Input` + `FormField` + `Button`; integra RHF via Controller (`value`/`onValueChange`). `state` p/ validação (não color variant). Uso: cor de Tags e Filas |
...
| **FileUploadField** | `ui/FileUploadField/file-upload-field.styles.ts` | `src/components/ui/FileUploadField/` | ✅ **v0.10.x** — captura de UM arquivo (dumb: só `File` + preview; consumer faz o upload). Dropzone clicável → preview (thumbnail img ou Chip nome+ícone) + remover. Composto de `FormField` + `Button` + `Chip` + `Icon`. `value: File\|string\|null`; valida accept/maxSizeMB (rejeita + `onError`). Drag&drop = fase 2. Uso: mídia de Quick Messages e Agendamentos |
...
| **MessageBubble** | `ui/MessageBubble/message-bubble.styles.ts` | `src/components/ui/MessageBubble/` | ✅ **v0.11.x (chat)** — bolha sent/received (tail), mídia embutida (image/audio/video/document/location/vcard), quoted reply, ack inline, isEdited/isDeleted. Compõe `MessageAck`+`MarkdownText`. `max-w-[70%]` (exceção). Atendimento/Chat interno |
| **MessageComposer** | `ui/MessageComposer/message-composer.styles.ts` | `src/components/ui/MessageComposer/` | ✅ **v0.11.x (chat)** — shell DUMB do input (toolbar slots, textarea auto-grow Enter=send, send/mic, slots reply/banner, state open\|disabled\|read-only). O app injeta comportamento (emoji/anexo/áudio/quick-msg) |
| **MessageAck** | `ui/MessageAck/message-ack.styles.ts` | `src/components/ui/MessageAck/` | ✅ **v0.11.x (chat)** — glifo de status (relógio→check→check-check, cor por ack 0-5 + estado error) |
| **MarkdownText** | `ui/MarkdownText/markdown-text.styles.ts` | `src/components/ui/MarkdownText/` | ✅ **v0.11.x (chat)** — markdown WhatsApp (`*bold*`/`_italic_`/`~strike~`/mono + autolink) SANITIZADO (parse manual p/ React nodes, sem dangerouslySetInnerHTML); `inline` = prévia truncável |
| **ConversationListItem** | `ui/ConversationListItem/conversation-list-item.styles.ts` | `src/components/ui/ConversationListItem/` | ✅ **v0.11.x (chat)** — item de lista de conversa (avatar/título/prévia/hora/badges/unread/selected/ações), variant `ticket`\|`chat`. Lista vertical (não-DataTable) |
| **DateSeparatorChip** | `ui/DateSeparatorChip/date-separator-chip.styles.ts` | `src/components/ui/DateSeparatorChip/` | ✅ **v0.11.x (chat)** — separador centralizado de data/fronteira na thread (variant `date`\|`boundary`) |
| **EmptyState** | `ui/EmptyState/empty-state.styles.ts` | `src/components/ui/EmptyState/` | ✅ **v0.11.x** — estado vazio genérico (ícone + título + descrição + ação, size sm\|md\|lg). Reusável no app todo |
| **MessageVariablesPicker** | `ui/MessageVariablesPicker/message-variables-picker.styles.ts` | `src/components/ui/MessageVariablesPicker/` | ✅ **v0.10.x** — picker de variáveis `{{...}}` que emite `onSelect(token)` (consumer insere no cursor; NÃO contém o textarea). Trigger "{ }" → Popover com Chips das variáveis. Composto de `Popover` + `Button` + `Chip` + `Icon`. `DEFAULT_MESSAGE_VARIABLES` = 5 (firstName/name/ms/protocol/hora, com trailing-space de paridade exceto firstName). Uso: corpo de Agendamentos e Quick Messages |
