...
| Avatar            | `ui/avatar-ig/avatar.styles.ts`                            | `src/components/ui/avatar-ig/`         | ✅ implementado (dir `avatar-ig` — distinto do primitivo shadcn `avatar`). Exporta também **`AvatarGroup`** (v0.53.0): pilha sobreposta com `size` propagado por contexto, `max`+`total` pro `+N`, e `surface` pra cor do anel (a superfície de TRÁS). Antes cada tela fazia `-ml-*`+`ring` na mão. O `Avatar` aceita `src` (foto) desde a mesma versão — iniciais viram fallback                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| **CardCheckbox**  | `ui/CardCheckbox/card-checkbox.styles.ts`                  | `src/components/ui/CardCheckbox/`      | ✅ **v0.7.1** — **atalho de `CardOption type="checkbox"`** desde v0.50.0 (a implementação mora em `ui/CardOption`; mantido porque está no registry, no barrel, no vocabulário e em 2 telas reais). Checkbox apresentado como card clicável (área grande, label + description visíveis). Mesma estética dos radio cards (bg-success-muted + border-brand no selected). Diferente de `FormFieldCheckbox` (compact horizontal). Props: `label` + `description` + opcional `icon` à esquerda. Toggle via clique no card inteiro (label htmlFor). Uso atual: SacarDialog aba "Outra conta" — opção "Salvar essa conta pra usar depois"                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| **CardOption**    | `ui/CardOption/card-option.styles.ts`                      | `src/components/ui/CardOption/`       | ✅ **v0.50.0** — checkbox, radio OU switch apresentado como card clicável (label + descrição). `type` troca o controle; `orientation` e `highlightSelected` DERIVAM do type (switch à direita, sem destaque, porque switch é estado e não seleção). `CardOptionGroup` com `layout="list"` faz lista de settings com divisórias, e é OBRIGATÓRIO em `type="radio"` (é ele que vira o RadioGroup do Radix). Tamanhos sm/md/lg. Unificou os 3 padrões que divergiam em 11 dimensões — o `CardCheckbox` virou atalho de `type="checkbox"` |
...
| Scheduler | `ui/Scheduler/scheduler.styles.ts` | `src/components/ui/Scheduler/` | ✅ implementado — calendário de EVENTOS (não confundir com `Calendar`, que é seletor de data). As 4 views implementadas: `month` (grade 6×7, `+N mais`, `+` de criar), `week`/`day` (a MESMA view `time-grid` com 7 ou 1 coluna — gutter de horas, banda de dia inteiro, lane-packing, linha do agora) e `list` (agenda só com os dias que têm evento). Toolbar com busca, filtros declarativos e painel-coluna de filtro. **A célula do mês adapta o corte do `+N mais` à altura MEDIDA da linha** — 76px→2 pills, 176px→6 — que é o que faz a versão de tela cheia (`#/scheduler-full`) aproveitar o espaço, como as linhas da tabela. **Drag & drop** (`@dnd-kit/core`, mesma base do Kanban): mover no mês muda a data preservando hora/duração, mover em week/day combina coluna + minutos snapados, e a borda do bloco redimensiona. Só liga com `onEventMove`/`onEventResize` conectado. **Teclado**: roving tabindex — cada grade é UMA parada de Tab. Núcleo puro testado: 51 casos em `layout.test.ts` + 19 em `use-scheduler-dnd.test.ts`. Núcleo puro (`hooks/layout.ts`: lane-packing, spans multi-dia, snap, resize) pronto e coberto por 36 testes. Dumb sobre mutação: emite `onEventMove`/`onEventResize`, nunca muta `events`. Dep: `date-fns` — **ainda não registrado no registry.json** (fecha no `/ds-release`) |
| Gantt | `ui/Gantt/gantt.styles.ts` | `src/components/ui/Gantt/` | ✅ implementado (núcleo) — cronograma de projeto com **VÍNCULOS**. Não confundir com `Scheduler` (eventos: quando algo acontece) nem com `Calendar`/`DatePicker` (escolher uma data). É o vínculo que distingue Gantt de timeline. Dois painéis com **uma fonte única de altura de linha** (`GANTT_ROW_HEIGHT_PX` = 48px — os 44 de `comp-2xl` + 2px de respiro em cima e embaixo) — medido: zero desalinho nas 11 barras contra 13 linhas. Grade própria e NÃO `DataTable` (12.187 linhas, `hierarchical` só na view de lista, altura vinda de `density` = L-038). Um tipo, dois formatos de dado: 1 barra + `parent` = Gantt de projeto; N barras sem `parent` = timeline de portfólio com `lanePacking` stack/compact. Os **4 tipos de vínculo** (FS/SS/FF/SF) com `lag`, detecção de conflito (borda tracejada + seta de perigo) e caminho crítico opt-in — ⚠️ crítico só considera `FS`, declarado, porque implementar os outros pela metade daria resultado plausível e errado. Setas em camada SVG **irmã** do canvas (filha da célula seria recortada pelo `overflow`, o defeito que o `DragOverlay` resolveu no Scheduler). Barra **tingida** com acento sólido e não sólida com texto branco — medido na v0.56.0: branco sobre pílula tingida dá 1.72–4.49 no light. Cor = CATEGORIA (`chart-1..5`, azul legítimo como dado); status vai em `row.trailing` como `Chip`. Dumb sobre mutação: emite `onBarMove`/`onLinkViolations`, nunca reagenda. Núcleo puro: **146 testes** (geometria + grafo + os 6 tipos de filtro). **Filtro aceita os 6 tipos** (`multi`/`single`/`text`/`number`/`date`/`boolean`), com o vocabulário de operador da `DataTable` e chips de aplicado abaixo da toolbar. As **três visões** estão montadas (`timeline`, `calendar` e `list`/agenda), com o seletor segmentado como PRIMEIRO item da toolbar + divisor (igual à `DataTable`), e os **4 gestos** ligados (mover, redimensionar, criar e remover vínculo) por pointer events nativos — não dnd-kit, porque o alvo aqui é coordenada e não zona. ⚠️ **Ainda NÃO**: setas de vínculo e gesto na grade de mês (decisão declarada, não débito). Deps: `date-fns`, `lucide-react`. Registry: item `gantt` com `@igreen/button`, `-checkbox`, `-dropdown-menu`, `-floating-panel`, `-popover`, `-radio-group`, `-tv`, `-utils` |
...
| ChoroplethMap | `ui/ChoroplethMap/choropleth-map.styles.ts` | `src/components/ui/ChoroplethMap/` | ✅ implementado — mapa coroplético data-driven: aceita GeoJSON `FeatureCollection`/`Feature[]` ou `Topology` TopoJSON, projeta com `d3-geo` (default `geoMercator().fitSize`) e colore por valor numa rampa sequencial derivada de token (`scaleToken` brand/success/info/warning/danger — nunca hex). Legenda, tooltip e `onFeatureClick` (gancho de drill-down UF→municípios). Deps próprias: `d3-geo` + `topojson-client` (L-037). Doc `#/choropleth-map`. **Quando NÃO usar:** mapa fixo de KPI do Brasil — para esse caso a receita de paths inline (`#/chart-map`) é mais leve e sem dependência |
| Kpi               | `ui/Kpi/kpi.styles.ts`                                     | `src/components/ui/Kpi/`               | ✅ implementado — primitivos composáveis de KPI: `Kpi` (card: label+ícone(tone)+valor+delta+hint+slot sparkline+footnote) · `KpiGroup` (layout columns/divided) · `KpiDelta` (pílula de variação sobre o Chip; `signed` deriva tom+seta do sinal). Doc viva `#/kpi` (Primitivos + API + Composições/9 modelos). **Composições canônicas de dashboard/lista: `.ai/context/components/dashboard-patterns.md`** (L-055). Theme-aware                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| Table             | `ui/Table/table.styles.ts`                                 | `src/components/ui/Table/`             | ✅ implementado; inclui `TableSpanRow` (linha de largura total — o `colSpan` que um grid de div não tem: use para "nenhum resultado", "carregando" e cabeçalho de grupo) |
| DatePicker        | `ui/DatePicker/datepicker.tsx`                             | `src/components/ui/DatePicker/`        | ✅ implementado — seletor de data(s) (composto Popover + Calendar), trigger estilo input do DS. **`mode`: `single` (default) · `range` · `multiple`** — união discriminada: o modo troca o TIPO de `value`/`onValueChange` (`Date` / `DateRange` / `Date[]`). `numberOfMonths` default 1, mas 2 em `range`. Ver `USAGE.md`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| ScreenLoader | `ui/ScreenLoader/screen-loader.styles.ts` | `src/components/ui/ScreenLoader/` | ✅ **v0.47.0 distribuído** — loading de página/área que preenche o pai (irmão do EmptyState): `variant="spinner"` (Spinner centrado + título + descrição, default) ou `variant="skeleton"` (silhueta genérica; `skeletonLayout`: page/dashboard/kpis). sizes sm/md/lg (md = padrão de uso). Registry `@igreen/screen-loader` (leva o USAGE.md junto). Doc `#/screen-loader` |
...
| **TabsNavigation** | `ui/TabsNavigation/tabs-navigation.styles.ts` | `src/components/ui/TabsNavigation/` | ✅ **v0.54.0** — abas de NAVEGADOR (sessão aberta: conversa, chamado, registro), compound: `TabsNavigation` + `.Tab` `.Title` `.Subtitle` `.Action` `.Actions` `.Panel`. Controlado (`value`/`onValueChange`) e **não hospeda o conteúdo** — painel pode morar fora (`panelId` → `aria-controls`). `density` (48/40px) · `fill` (altura total, une por cor em vez de régua) · `actionsMode` · overflow com setas + lista. Não confundir com o `tabs` shadcn, que troca miolo dentro da tela |
| **Breadcrumb** | `ui/Breadcrumb/breadcrumb-switcher.styles.ts` | `src/components/ui/Breadcrumb/` | ✅ **v0.55.0** — re-exporta os primitivos do `shadcn/breadcrumb` **+** o `BreadcrumbSwitcher`: o item do caminho que TROCA o registro aberto (padrão do seletor de repo do GitHub), com dropdown de busca (`Command`), `leading`/`description`/`keywords`/`group` por opção e `footer` fixo. Controlado, **não navega**. Integrado ao `Header` (item vira seletor com `switcher`+`value`+`onValueChange`). **Um item de registry só** (`breadcrumb`) — o caminho e a variação são a mesma peça |
...
src:      string (foto; URL que falha volta pras iniciais de children)
```

- Non-interactive (no focus, no disabled).
- `src` cobre o círculo (`object-cover`) e escala pelos mesmos sizes; sem prop `alt` — a
  imagem é `alt=""` e o nome fica no `aria-label` do avatar.
