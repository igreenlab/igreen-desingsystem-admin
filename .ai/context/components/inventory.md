...
| **BreadcrumbSwitcher** | `ui/BreadcrumbSwitcher/breadcrumb-switcher.styles.ts` | `src/components/ui/BreadcrumbSwitcher/` | ✅ **vNEXT** — item do breadcrumb que TROCA o registro aberto (padrão do seletor de repo do GitHub): gatilho com a tipografia do caminho + dropdown com busca (`Command`), `leading`/`description`/`keywords`/`group` por opção, `footer` fixo. Controlado e **não navega**. Integrado ao `Header`: item do breadcrumb vira seletor com `switcher`+`value`+`onValueChange`. Não confundir com `combobox` (campo de formulário) |
