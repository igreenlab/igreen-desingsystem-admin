...
### [2026-07-02] | DS DEV | 4 gaps do Rankings: Spinner + Tooltip + MonthYearPicker + ChoroplethMap | CONCLUÍDO
- Input: a migração do módulo Rankings precisa de 4 componentes ausentes no DS. Dono do DS APROVOU criar+mergear (gate humano pulado por autorização explícita), exigindo seguir padrões DS + checklist do reviewer. Branch `feat/combobox-field-picker`.
- Output / arquivos (iGreen = 5 arquivos: index.ts + .tsx + .styles.ts + .types.ts + USAGE.md):
  - NOVO `ui/Spinner/` — loading (SVG `animate-spin` + `motion-reduce:animate-none`). `size` sm/md/lg (`size-icon-*`), `color` current/default/muted/brand/on-brand (`text-fg-*`, traço `currentColor`), `role="status"`+`label` (ou `aria-hidden`).
  - NOVO `shadcn/tooltip.tsx` — Radix Tooltip (`@radix-ui/react-tooltip`, dep NOVA) remapeada: bolha `bg-bg-emphasis`+`text-fg-default`+`text-caption-sm`+`rounded-radius-md`+`shadow-sh-md`, seta `fill-bg-emphasis`. `<Tooltip>` auto-embrulha `<TooltipProvider>`. `<TooltipContent showArrow?>`.
  - NOVO `ui/MonthYearPicker/` — período mês+ano (`"YYYY-MM"`). `Popover` + navegação de ano + grade 3×4 (Intl pt-BR). Trigger espelha `SelectTrigger`. `min`/`max` desabilitam meses.
  - NOVO `ui/ChoroplethMap/` — mapa coroplético genérico data-driven. `geography` (GeoJSON/TopoJSON) + `values` (`id→nº`). Escala default = `color-mix(bg-{scaleToken} → bg-surface)` (fill inline data-driven, exceção tipo L-027); `colorScale` custom opcional. Wrapa `d3-geo`+`topojson-client` (deps NOVAS) — **não** `react-simple-maps`. Hover reusa o `Tooltip` do DS (ancorado no centroide; 1 tooltip controlado que segue o cursor). Legenda de gradiente.
  - MOD `shadcn/index.ts` + `components/index.ts` (exports). MOD `inventory.md` (ui/ 21→24, shadcn/ 26→27, remove Tooltip de "planejados"). MOD `package.json` (deps).
- Dependências novas: `@radix-ui/react-tooltip@^1.2.11` (limpo, React 19 ok). `d3-geo@^3.1.1` + `topojson-client@^3.1.0` (+ `@types/d3-geo`, `@types/topojson-client` dev). **`react-simple-maps` NÃO instalada**: v3.0.0 trava peer em `react@^16.8||17||18` → ERESOLVE em React 19; `--legacy-peer-deps`/`--force` seria hack e contaminaria a árvore de todos os consumidores do DS. Solução limpa: usar as MESMAS primitivas que a react-simple-maps embrulha (d3-geo + topojson-client), sem peer quebrado.
- Tokens novos: NENHUM (cascata não necessária). Só existentes, confirmados no `tailwind-theme.css` gerado + no CSS buildado: `size-icon-sm/md/lg`, `text-fg-*`, `text-current`, `bg-bg-emphasis`+`fill-bg-emphasis`, `text-caption-sm/xs`, `rounded-radius-md/lg/base/full`, `shadow-sh-md`, `min-h-form-lg/md`, `size-form-sm`, `bg-bg-input(/-hover)`/`bg-bg-muted(/-hover)`, `border-border-input/brand/default`, `bg-bg-brand(/-hover)`+`text-fg-on-brand`, `ring-ring-brand`, `shadow-sh-ring`, `stroke-border-default`+`hover:stroke-fg-brand`, `h-gp-md`. Fill do mapa via `color-mix(in oklch, var(--color-bg-{token}) N%, var(--color-bg-surface))` — endpoints são tokens, N% vem do dado (exceção = Avatar.colorHex/L-027). `w-[16rem]`/`max-w-[16rem]` são rem (permitido).
- Validação: `npm run build` (DS) VERDE (tokens:tw4 + tsc -b + vite build, 4243 módulos). Classes-chave confirmadas no CSS buildado (incl. `fill-bg-emphasis`, `size-form-sm`, `motion-reduce:animate-none`). Showcase/doc-page no preview NÃO adicionada (dev server 3100 quebra no optimizeDeps do lucide-react — defeito conhecido das entries anteriores; só o dev optimizer, não o prod build) — FOLLOW-UP.
- Assumption: (a) o DS não tem surface invertida → tooltip como bolha neutra `bg-bg-emphasis` é a escolha token-nativa, sem token novo; (b) reusar o Tooltip via 1 âncora 0×0 no centroide (controlado por `open`) escala p/ milhares de `<path>` — 1 Tooltip por região seria inviável; (c) fill data-driven inline (color-mix com endpoints-token) é a única forma p/ escala contínua e é consistente com L-027; (d) react-simple-maps é só um wrapper fino de d3-geo+topojson-client → construir sobre as primitivas é equivalente em render e melhor em compat (React 19), não um downgrade.
- Lições novas: candidata — "Dep com peer travado em React ≤18 num DS React 19 (ex.: react-simple-maps): NÃO forçar com --legacy-peer-deps (contamina a árvore de todo consumidor) — preferir as primitivas que a lib embrulha (d3-geo/topojson-client). + 'não há surface-inverted no DS: tooltip = bolha bg-emphasis'." Avaliar no review.
